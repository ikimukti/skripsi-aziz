            <aside class="bg-gray-800 text-white w-64 overflow-y-scroll h-screen flex-shrink-0 sc-hide">
                <ul class="text-gray-400">
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-tachometer-alt mr-3"></i>
                        <a href="../systems/dashboard.php">Dashboard</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-user mr-3"></i>
                        <a href="../profiles/profile.php">Profile</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-users mr-3"></i>
                        <a href="../classes/classesList.php">Classes</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-user-graduate mr-3"></i>
                        <a href="../students/studentsList.php">Students</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-chalkboard-teacher mr-3"></i>
                        <a href="../teachers/teachersList.php">Teachers</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-book mr-3"></i>
                        <a href="../subjects/subjectsList.php">Subjects</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-book-reader mr-3"></i>
                        <a href="../materials/materialsList.php">Materials</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-file-alt mr-3"></i>
                        <a href="../exams/examsList.php">Exams</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="far fa-calendar-alt mr-3"></i>
                        <a href="../schedules/schedulesList.php">Schedules</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="far fa-calendar-check mr-3"></i>
                        <a href="../attendances/attendancesList.php">Attendances</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center bg-gray-700">
                        <i class="fas fa-user-cog mr-3"></i>
                        <a href="../users/usersList.php">Users</a>
                    </li>
                    <li class="px-6 py-4 hover:bg-gray-700 cursor-pointer space-x-2 flex items-center">
                        <i class="fas fa-cog mr-3"></i>
                        <a href="../systems/settingsList.php">Settings</a>
                    </li>
                </ul>
                <hr class="mt-60">
            </aside>